...
def build_scan_discovery_frame() -> bytes:
    """Build scan discovery frame."""
    handler = ProtocolHandler()
    return handler.build_frame(0x10)  # OP_START_SCAN


def build_stop_scan_frame() -> bytes:
    """Build stop scan frame."""
    handler = ProtocolHandler()
    return handler.build_frame(0x11)  # OP_STOP_SCAN


def build_factory_reset_frame() -> bytes:
    """Build factory reset frame."""
    handler = ProtocolHandler()
    return handler.build_frame(0x20)  # OP_FACTORY_RESET
...
def build_device_status_query_frame(network_addr: int, msg_type: int = 0) -> bytes:
    """Build device status query frame."""
    handler = ProtocolHandler()
    # 状态查询命令: 地址(2字节) + msg_type(1字节，0表示查询所有)
    payload = network_addr.to_bytes(2, 'little') + bytes([msg_type])
    return handler.build_frame(0x32, payload)  # OP_DEVICE_STATUS_QUERY


def build_scene_control_frame(scene_id: int) -> bytes:
    """Build scene control frame."""
    handler = ProtocolHandler()
    # 场景控制命令: 场景ID(1字节)
    payload = bytes([scene_id])
    return handler.build_frame(0x31, payload)  # OP_SCENE_CONTROL


def build_device_control_frame(device_addr: int, msg_type: int, param: int) -> bytes:
    """Build device control frame."""
    handler = ProtocolHandler()
    # Device control: address(2 bytes) + msg_type(1 byte) + param(1 byte)
    addr_bytes = device_addr.to_bytes(2, 'little')
    data = addr_bytes + bytes([msg_type, param])
    return handler.build_frame(0x30, data)  # OP_DEVICE_CONTROL


def build_add_device_frame(mac_address: bytes, network_addr: int = 0) -> bytes:
    """Build add device frame."""
    handler = ProtocolHandler()
    # Add device: MAC(6 bytes) + network_addr(2 bytes)
    addr_bytes = network_addr.to_bytes(2, 'little')
    data = mac_address + addr_bytes
    return handler.build_frame(0x12, data)  # OP_ADD_DEVICE
